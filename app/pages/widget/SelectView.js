...
class SelectView extends React.Component {
    ...
    getOptiontValue(props) {
        var value = (props.options.length === 0 ? "加载中..." : props.options[0].value);
        console.log("SelectView getOptiontValue value: " + value + ", options.size: " + props.options.length);
        return value;
    }

    getOptionViews(props) {
        var options = props.options;
        console.log("SelectView getOptionViews options.size: " + options.length);
        var optionViews = [];

        for(var index = 0; index < options.length; index++) {
            var view  = <Option key={options[index].value}
